...
export default function MainLayout({ children, showSidebar = false }: MainLayoutProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <div 
        className={`hidden lg:block fixed inset-y-0 left-0 z-20 transition-all duration-300 pt-16`}
      ...
      <div 
        className={`lg:hidden fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 ease-in-out pt-16 ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{width: '16rem'}}
      >
